...
# Get the FailedTestTable name from CloudFormation
table_name=$(get_table_name "FailedTestTable")

if [ -z "$table_name" ]; then
    echo "Error: Could not find FailedTestTable in stack outputs"
    exit 1
fi
...
while :; do
    ...
    if [ -z "$exclusive_start_key" ]; then
        if ! scan_output=$(aws dynamodb scan --table-name "$table_name" --projection-expression "#usr,TestTime" --expression-attribute-names '{"#usr": "User"}' --output json 2>&1); then
            echo "Error: Failed to scan table '$table_name'"
            echo "$scan_output"
            exit 1
        fi
    else
        if ! scan_output=$(aws dynamodb scan --table-name "$table_name" --exclusive-start-key "$exclusive_start_key" --projection-expression "#usr,TestTime" --expression-attribute-names '{"#usr": "User"}' --output json 2>&1); then
            echo "Error: Failed to scan table '$table_name'"
            echo "$scan_output"
            exit 1
        fi
